#!/usr/bin/env python3
"""
PX4 Offboard 搜索 + 逼近 + 抛物线投放 + 下视急停 + 下视立即投放 + 自动返航 v1.6.3

新增（相对于 v1.6.2）：
- 下视摄像头只要检测到目标 Tag（经过计数阈值）立即触发舵机投放，同时执行急停并返航
- 移除了原版本中下视稳定检测 1 秒的等待
- LOST_COASTING 状态下下视检测到 Tag 同样立即投放+返航
- 进入 RETURN_TO_LAND 时强制将期望位置锁定为当前位置，实现“急停”效果
"""
...
class RobustSearchMission(Node):
    def __init__(self):
        ...
        self.declare_parameter('tag_detect_threshold', 1)
        ...
        self.get_logger().info("🚀 v1.6.3：下视立即触发投放+急停返航")
    ...
    def control_loop(self):
        try:
            ...
            # ========== 前视丢失后的滑行减速（增加抛物线投放判断）==========
                ...
                # ========== v1.6.3 修改：下视检测到 Tag 立即投放并返航 ==========
                with self.data_lock:
                    bottom_det = self.bottom_tag_detected
                if bottom_det and not self.servo_triggered and not self._servo_trigger_in_progress:
                    self.get_logger().info("🛑 下视检测到 Tag，立即投放并急停返航")
                    self.trigger_servo_drop()
                    self._enter_state(FlightState.RETURN_TO_LAND)
                    return

                # 滑行中继续尝试抛物线投放（基于最后已知距离和速度估算当前剩余距离）
                if not self.servo_triggered and not self._servo_trigger_in_progress:
                    # 估算已滑行距离
            ...
            # ========== v1.6.3 修改：统一的下视立即投放逻辑（在任何非返航/紧急状态下） ==========
            # 检查是否应该由下视直接触发投放+急停
            if (not self.servo_triggered and not self._servo_trigger_in_progress and
                current_state not in (FlightState.RETURN_TO_LAND, FlightState.EMERGENCY_LAND, FlightState.LANDING)):
                with self.data_lock:
                    bottom_det = self.bottom_tag_detected
                if bottom_det:
                    self.get_logger().info("✅ 下视检测到 Tag，立即投放并急停返航")
