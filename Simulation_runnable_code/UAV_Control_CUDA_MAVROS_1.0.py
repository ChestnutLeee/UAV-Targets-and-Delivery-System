#!/usr/bin/env python3
"""
基于机器视觉的无人机动态靶标精准识别与投送系统 —— 实机控制节点（安全修正版）
硬件平台: Jetson Orin Nano + Holybro Pixhawk4 + 双 CIS 摄像头
依赖: ROS2 (Humble), MAVROS, dt_apriltags, OpenCV, numba, pycuda (可选)
修正要点:
1. 视觉位姿坐标系正确转换 (相机->机体->ENU)
2. 增加卡尔曼滤波平滑位姿，防止飞控 EKF 崩溃
3. 速度控制时闭环调节高度，消除漂移
4. 监控 Offboard 模式，丢失时紧急降落
5. 控制模式切换平滑处理
6. 前视摄像头安装角修正
7. 完善投放逻辑与安全保护
"""
...
class IntegratedDroneMission(Node):
    def __init__(self):
        ...
        self.get_logger().info("🛸 实机系统启动 - 安全修正版")
        ...
        self.front_tag_pos = np.zeros(3)          # 机体坐标系下标签位置
        ...
        self.height_est = 2.5
        self.raw_drone_pos = np.zeros(3)          # 解算出的无人机ENU位置
        self.drone_pos_filtered = np.zeros(3)     # 滤波后的位置
        ...
        # 相机外参: 前视摄像头 (45°前下方安装)
        self.angle_front = math.radians(45)  # 光轴与机体X轴夹角，向下为正
        # 旋转矩阵: 绕机体Y轴旋转 -angle (使光轴由水平转到向下)
        c = math.cos(-self.angle_front)
        s_sq = math.sin(-self.angle_front)
        self.R_front_cam2body = np.array([[c, 0, s_sq], [0, 1, 0], [-s_sq, 0, c]])

        # 底部摄像头: 垂直向下，与机体坐标系一致，无需旋转
        self.R_bottom_cam2body = np.eye(3)

        # ---------- 控制参数 ----------
        self.target_altitude = 2.5          # 起飞高度 (ENU, 向上为正)
        ...
        self.pid_alt = PIDController(kp=0.8, ki=0.1, kd=0.2, integral_limit=0.3)  # 高度PID
        ...
        self.max_vision_lost_duration = 15.0  # 连续无视觉超过此时间则返航
    ...
    def process_image(self, frame):
        ...
        if NUMBA_CUDA_AVAILABLE:
            ...
            d_output = numba_cuda.device_array((480, 640), dtype=np.uint8)
            threadsperblock = (16, 16)
            blockspergrid_x = math.ceil(480 / threadsperblock[0])
            blockspergrid_y = math.ceil(640 / threadsperblock[1])
            ...
            try:
                ...
                with self.data_lock:
                    if results:
                        ...
                        if success:
                            # 相机坐标系 -> 机体坐标系
                            t_cam = tvec.flatten()                         # X_c, Y_c, Z_c (OpenCV标准)
                            t_body = self.R_front_cam2body @ t_cam         # 标签在机体坐标系下的位置
                ...
                with self.data_lock:
                    if results:
                        ...
                        success, rvec, tvec = cv2.solvePnP(self.tag_3d_pts,
                                                           ...
                                                           self.camera_matrix, self.dist_coeffs)
                        if success:
                            # 相机坐标系 (垂直向下) -> 机体坐标系 -> ENU世界
                            t_cam = tvec.flatten()
                            t_body = self.R_bottom_cam2body @ t_cam          # 标签在机体坐标系位置
                            # 机体->世界旋转矩阵
                            ...
                            # 假设标签位于ENU原点(0,0,0)，无人机ENU位置 = -R * t_body
    ...
    def start_sequence(self):
        ...
        if not self.request_offboard_mode():
            self.get_logger().error("无法进入 Offboard，放弃起飞")
            return
        time.sleep(1.0)
        ...
        self.get_logger().info("✅ 解锁完成，进入 TAKEOFF 状态")
    ...
    def watchdog_check(self):
        if time.time() - self.last_offboard_heartbeat > 0.6:
            self.get_logger().error("⏰ 看门狗超时，触发紧急降落！")
    ...
    def control_loop(self):
        ...
        # 检查是否仍处于 Offboard 模式，否则立即重新请求或降落
        if self.vehicle_state.mode != "OFFBOARD" and not self.emergency_land_triggered:
            self.get_logger().warn(f"退出 Offboard 模式 (当前: {self.vehicle_state.mode})，尝试重新激活")
            if not self.request_offboard_mode():
                self.get_logger().error("无法恢复 Offboard，紧急降落！")
                self.trigger_emergency_land()
                return
        ...
        if self.state not in [FlightState.TAKEOFF, FlightState.DROP]:
            ...
            if self.vision_lost_time > self.max_vision_lost_duration:
                self.get_logger().error("视觉丢失超时，启动返航降落")
                self.trigger_emergency_land()
                return
        ...
        try:
            ...
            # ---------- 逼近 ----------
                ...
                with self.data_lock:
                    ...
                    dist_f = self.front_tag_pos[0]   # 机体前向距离
            ...
            # ---------- 追踪 & 投放判断 ----------
                ...
                with self.data_lock:
                    ...
                    if self.bottom_tag_detected:
                        ...
                        yaw_err = abs(math.atan2(v_side[1], v_side[0])) if self.bottom_tag_corners is not None else 0.0
                        ...
                        if self.drop_ready_time >= self.drop_required_duration:
                            self.get_logger().info("✅ 投放条件满足，执行投放！")
                            self.trigger_drop()
                            self.state = FlightState.DROP
                            self.drop_ready_time = 0.0
                    else:
                        if time_since_bottom > self.tag_timeout_bottom:
                            self.get_logger().warning("完全丢失目标 → SEARCHING")
                            self._reset_all_pid()
                            self.state = FlightState.SEARCHING
                        else:
                            self.cmd_vx *= 0.95
                            self.cmd_vy *= 0.95
                            self.cmd_yawspeed *= 0.9
